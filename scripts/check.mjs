...
for(const file of ['index.html',...(await readdir('projects')).filter(f=>f.endsWith('.html')).map(f=>'projects/'+f)]){
 const html=await readFile(file,'utf8');
 assert.match(html,/<html lang="it">/);assert.match(html,/<h1[ >]/);assert.match(html,/rel="canonical"/);
